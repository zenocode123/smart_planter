...
這份文件定義了「智慧盆栽 (Smart Plant Pot)」專案的開發標準、技術架構與約束條件。在生成程式碼或提供架構建議時，請嚴格遵守以下規範。

## 1. 專案概述

一個結合 **Raspberry Pi 5** 與 **ESP32** 的智慧系統。樹莓派作為中央網關，負責高層邏輯與 AI；ESP32 負責感測器讀取與硬體控制。具備 AI 人格對話、自動化照顧與遠端監控功能。
...
- **資料庫**: **SQLite** (簡單、零配置)。使用 `SQLAlchemy` 或 `Tortoise-ORM`。
