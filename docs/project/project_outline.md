...
現代人因工作繁忙、空間限制等因素難以飼養寵物，轉而選擇種植植物，卻又常因缺乏時間或知識導致植物枯萎。同時，都市族群對於「情感陪伴」有強烈需求，我們希望透過 AIoT 技術打破人與植物間的溝通隔閡。

於是便有了此專案的誕生來讓植物像是擁有寵物般的情感，讓主人與植物之間有了更深刻的情感連結。
...
- **感測與控制節點 (MCU)**：
  - ESP32 (負責讀取感測器數值、控制馬達，透過 Serial 與 Pi 溝通)
...
> **架構備註**：採用 **去中心化網頁架構 (Decentralized Web Architecture)**。每台裝置皆為獨立運作的 Web Server，多裝置管理透過前端的「鄰近裝置連結」進行切換，無需中央雲端伺服器。
...
- **後端開發**：
  ...
  - 資料庫：SQLite (輕量化儲存，單機運作)
  - ORM：SQLAlchemy 或 Tortoise-ORM
  - **服務發現 (Service Discovery)**: 使用 **mDNS (Zeroconf)** 協定，實現區網內多裝置自動偵測與列表更新。
...
- **通訊協定**：
  - ESP32 <-> Pi：UART (Serial)
  - User <-> Pi：HTTP (Local Area Network)
  - Pi <-> Pi：HTTP (Peer-to-Peer API)
